...
def armysetup():
    ...
    army = dict()
    army[type] = (raw_input("Human? Elf? Dwarf? Orc? Robots? etc."))

    print
    army[fnum] = int(raw_input("How many foot soldiers are in your army?"))

    print
    army[anum] = int(raw_input("How many archers are in your army?"))

    print
    army[cnum] = int(raw_input("How many horsemen are in your army?"))

    print
    army[snum] = int(raw_input("How many artillery pieces are in your army?"))

    print
    print "One supply unit will keep a soldier fed and fit for one day."
    army[supp] = int(raw_input("How many days worth of supplies does your army have?"))
...
def deathisinevitable(deathtoll):
    ...
    #army = [10, 20, 30, 40, 50, 60]
    #deathtoll = 15

    transport = {"nothing1": 0, "nothing2": 0, "Footsoldiers": 0, "Archers": 0, "Cavalry": 0, "Siege weapons": 0}
...
def dangterrain(army):
    ...
    if terrain == "Water":
        ...
        print "Because of the size and strength of the current, it took %s minutes to traverse the water" % ((10 * size) + ((size + 10) * flow))
